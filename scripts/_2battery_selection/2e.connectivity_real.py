...
# load data
data_mdtb_s2_run_suit,info_mdtb_2_run  =MDTB_dataset.get_data(space=space,ses_id='ses-s2',type='CondRun',subj=subj)
...
data_mdtb_s2_run_fs,info_mdtb_2_run  =MDTB_dataset.get_data(space=space,ses_id='ses-s2',type='CondRun',subj=subj)
...
ncond = info_mdtb_2_run['cond_num'].nunique()
npart = info_mdtb_2_run['run'].nunique() 

# Get crossvalidated second moment matrix (GLib)
G_lib =  ct.get_G(data=data_mdtb_s2_run_fs,n_cond=ncond,n_part=npart)
...
save_path = os.path.join(save_dir, 'connectivity_real_2t1_10000.tsv')
