...
comb_names, Uhats_multi_full, Uhats_multi_collapsed =ev.real_localization_multi(G_Lib,combination=[7,9,13,16],task_names_s1=task_names_s1,
...
# style maps
colors = {'Inside': 'darkorange', 'Outside': 'steelblue'}
...
x_tick_positions = []
start = 0
for g_i, group in enumerate(group_order):
    group_center_positions = []
    for r_i, region in enumerate(region_order):
        for c_i, contrast in enumerate(contrast_order):
            key = (group, contrast, region)
            offset = (r_i * 2 + c_i - 1.5) * bar_width
            x = start + offset
            x_positions[key] = x
            group_center_positions.append(x)
    x_labels.append(group)
    x_tick_positions.append(np.mean(group_center_positions))
    start += group_gap
...
for _, row in df.iterrows():
    ...
    x = x_positions[key]
    ax.bar(
        x, row['Value'], yerr=row['SEM'], width=bar_width,
...
ax.set_title('Contrast by Region and Localizer Type')

sns.despine()

# legend: one for region colors, one for contrast hatches
legend = [
    Patch(facecolor='darkorange', edgecolor='black', label='Inside'),
    Patch(facecolor='steelblue', edgecolor='black', label='Outside'),
...
plt.tight_layout()
if save_plot:
    fig.savefig(f"{save_dir}/single_vs_multi_real/contrasts.pdf",bbox_inches='tight')
if get_pdf:
    pdf.savefig(fig)
    plt.close(fig)
