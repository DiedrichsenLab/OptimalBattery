...
# DCBC voxel correlations use held-out conditions only
loc_conds = set(multi_combo) | set(single_contrast_names)
heldout_idx = np.where(~info_all['task_name'].isin(loc_conds).values)[0]
print(f"DCBC held-out conditions ({len(heldout_idx)}): {list(info_all['task_name'].iloc[heldout_idx])}")
...
plt.tight_layout()
fig.savefig(f"{save_dir}/single_vs_multi/real_dcbc_barplot.pdf", bbox_inches="tight")
plt.show()
