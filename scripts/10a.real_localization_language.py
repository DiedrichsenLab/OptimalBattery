...
data_run,info_run  =lang_dataset.get_data(space=space,ses_id='ses-localizer_cond',type='CondRun',subj=subj)
...
info_run['taskName'] = info_run['taskName'].str.replace(' ', '')
info_all['taskName'] = info_all['taskName'].str.replace(' ', '')

data_run = ut.recenter_fmri_data(data_run,info_run,task_column_name='taskName',center_condition='rest')
...
condition_df= ct.get_condition_indices(info_run, task_column_name='taskName', cond_column_name='taskName')
...
bar_offsets = {
    ('Multitask', 'Intact>Degraded'): -0.3,
    ('Multitask', 'n-back>Rest'): -0.1,
    ('Single-task', 'Intact>Degraded'): 0.1,
    ('Single-task', 'n-back>Rest'): 0.3
}
