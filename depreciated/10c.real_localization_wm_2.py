...
pdf_name = 'localization_cerebellum_wm_2.pdf'
...
data_run,info_run  =dataset.get_data(space=space,ses_id='ses-s1',type='CondRun')
data_run[np.isnan(data_run)] = 0

data_all,info_all  =dataset.get_data(space=space,ses_id='ses-s1',type='CondAll')
data_all[np.isnan(data_all)] = 0

data_all_test , info_all_test = dataset.get_data(space=space,ses_id='ses-s2',type='CondAll')
...
region_mapping = {
    ...
    6: ['A1R', 'A2R', 'A3R'],
    7: [ 'D2R', 'D3R'],
    8: ['D1R','D4R'],  # target region
    9: ['S1R', 'S2R','S3R', 'S4R', 'S5R']}
...
smaller_ROIs = [8,9]
ROI_mask_smaller = np.isin(coarse_parcelation, smaller_ROIs).astype(int)
ROI_indices_smaller = np.where(ROI_mask_smaller == 1)[0]


# Get the G_library for task selection
n_conds = len(task_names_s1)
n_part = data_run.shape[1] // n_conds
G_Lib = ct.get_G(data=data_run[:,:,ROI_indices_smaller],n_cond=n_conds,n_part=n_part)
if get_pdf:
    plt.imshow(G_Lib)
...
comb_names, Uhats_multi_full, Uhats_multi_collapsed =ev.real_localization_multi(G_Lib,combination=None,task_names_s1=task_names_s1,
...
single_contrast_regs = ev.find_single_contrast(full_vs_train,3,4)
